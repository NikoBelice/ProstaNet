...
for fold, (train_indices, val_indices) in enumerate(kfold.split(dir_dataset)):
  ...
  trainloader = DataLoader(dataset=trainset, batch_size=8, num_workers=0, shuffle=True, drop_last=True) #batch_szie is adjustable
  validateloader = DataLoader(dataset=validateset, batch_size=8, num_workers=0, shuffle=True, drop_last=True)
  ...
  model = staGCNN()
  pretrained_dict = torch.load(f"../model/GCNs15.pth")
  ...
  for param in model.wild_conv2.parameters():
    param.requires_grad = False
  for param in model.mutant_conv2.parameters():
    param.requires_grad = False
  for param in model.wild_fc1.parameters():
    param.requires_grad = False
  for param in model.mutant_fc1.parameters():
    param.requires_grad = False
  for param in model.wild_conv1.parameters():
    param.requires_grad = False
  for param in model.mutant_conv1.parameters():
    param.requires_grad = False

  optimizer =  torch.optim.Adam([
  {'params': model.wild_MLP.parameters(), 'lr': 1e-3, 'weight_decay': 1e-5}
  ...
  for epoch in range(num_epochs):
    ...
    if(val_loss < best_loss):
      ...
      torch.save(model.state_dict(), f"../model/GCNm2{k}.pth") #path to save the model
      print("Model")
