...
for fold, (train_indices, val_indices) in enumerate(kfold.split(dir_dataset)):
  ...
  trainloader = DataLoader(dataset=trainset, batch_size=16, num_workers=0, shuffle=True, drop_last=True)
  validateloader = DataLoader(dataset=validateset, batch_size=16, num_workers=0, shuffle=True, drop_last=True)
  ...
  model = StaGVP((91, 3), (int(2*91), int(1*3)), (32, 1), (32, 1), 0.6)

  pretrained_dict = torch.load(f"../../model/GVPs104.pth")
  ...
  optimizer = torch.optim.Adam([
    {'params': model.wild_conv1.parameters(), 'lr': 1e-4, 'weight_decay': 1e-4},
    {'params': model.mutant_conv1.parameters(), 'lr': 1e-4, 'weight_decay': 1e-4}
    ])
  ...
  for epoch in range(num_epochs):
    ...
    if(val_loss < best_loss):
      ...
      torch.save(model.state_dict(), f"../../model/GVPm11{k}.pth") #path to save the model
      print("Model")
